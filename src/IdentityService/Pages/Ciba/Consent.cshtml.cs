...
public class Consent : PageModel
{
  ...
  public async Task<IActionResult> OnPost()
  {
    ...
    if (request == null
     || request.Subject.GetSubjectId() != User.GetSubjectId())
    {
    ...
    else if (Input?.Button == "yes")
    {
      ...
      if (Input.ScopesConsented != null
       && Input.ScopesConsented.Any())
      {
        var scopes = Input.ScopesConsented;

        if (ConsentOptions.EnableOfflineAccess == false)
        {
          scopes = scopes.Where(x => x
      ...
      else { ModelState.AddModelError("", ConsentOptions.MustChooseOneErrorMessage); }
    }
    else { ModelState.AddModelError("", ConsentOptions.InvalidSelectionErrorMessage); }
  ...
  async private Task<ViewModel> BuildViewModelAsync(string id, InputModel model = null)
  {
      ...
      BackchannelUserLoginRequest request)
  {
    ...
    var resourceIndicators =
        request.RequestedResourceIndicators ?? Enumerable.Empty<string>();

    var apiResources =
        request.ValidatedResources.Resources.ApiResources.Where(x => resourceIndicators
            .Contains(x.Name));
    ...
    foreach (var parsedScope in request.ValidatedResources.ParsedScopes)
    {
      ...
      if (apiScope != null)
      {
        var scopeVm = CreateScopeViewModel(parsedScope,
                                           apiScope,
                                           model == null
                                        || model.ScopesConsented?.Contains(parsedScope
                                               .RawValue)
                                        == true);
