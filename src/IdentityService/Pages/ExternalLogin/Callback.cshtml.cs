...
public class Callback : PageModel
{
  ...
  async private Task<ApplicationUser> AutoProvisionUserAsync(
      string             provider,
      ...
      IEnumerable<Claim> claims)
  {
    ...
    if (name != null) { filtered.Add(new Claim(JwtClaimTypes.Name, name)); }
    else
    {
      ...
      else if (first != null) { filtered.Add(new Claim(JwtClaimTypes.Name, first)); }
      ...
      AuthenticationProperties localSignInProps)
  {
    ...
    localClaims.Add(new Claim(JwtClaimTypes.IdentityProvider,
                              externalResult.Properties.Items["scheme"]));

    // if the external system sent a session id claim, copy it over
    // so we can use it for single sign-out
    var sid =
        externalResult.Principal.Claims.FirstOrDefault(x => x.Type
